(function ($) {
    'use strict';
    Drupal.behaviors.igcommerce_paginate = {
            ...
            function limitVisible() {
              $('.paginate', context).on().each(function () {  
                  ...
                  if(items >= (shown + 1)) {
                      ...
                      paginationLinks = paginationLinks +  '<button type="button" class="previous_page hidden" value="Previous Page">' + previousText + '</button>';
                      ...
                      paginationLinks = paginationLinks + '<button type="button" class="next_page" value="Next Page">' + nextText + '</button>';
                      
                      var showAll = Drupal.t('Show all');
                      paginationLinks = paginationLinks + '<span class="toc_show_all"><button type="button" value="Show All">' + showAll + '</button></span>';
                  }
                  ...
                  $(this).on("click",  "a.toc-page-link",  function(e) {
                      ...
                      if (typeof(totalPages) != "undefined" && totalPages !== null) {
                          ...
                          if ((showNextHighestItemNumber / shown)  % 1 == 0 && parseInt($(this).text()) > 1) {
                              var lastItem = $(this).parents('.paginate').find('.row:visible:last');
                              ...
                              $(this).parents('.paginate').find(".row:nth-child("+ lowestNumberToShow +")").nextUntil('.row:nth-child('+ (showNextHighestItemNumber + 1) +')' , "li.row" ).show();
                          }
                          ...
                          if (totalPagesFloat % 1 != 0 && floatNumber < shown && parseInt($(this).text()) === items) {
                              ...
                              $(this).parents('.paginate').find('.row:nth-last-child(-n+' + floatNumber + ')').show();
                          }
                          currentPage = $(this).text();
                      }
                      $(this).parent().find(".toc-page-link").removeClass('list_active_page');
                      $(this).addClass('list_active_page');
                      // Sends the user back to the top of the parent .paginate
                      $('html, body').animate({scrollTop: $(this).closest(".paginate").offset().top -144 }, 'slow');
